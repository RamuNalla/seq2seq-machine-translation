...
class Encoder(nn.Module):
    """
    LSTM Encoder - Reads input sequence and produces final hidden/cell states as context
    """
    ...
    def forward(self, src: torch.Tensor, src_lengths: torch.Tensor = None):
        ...
        embedded = self.embedding(src)  # # Embed source sequences: (batch_size, src_len, embedding_dim)
        ...
        # Pass through LSTM. We only need the final hidden and cell states (the context vector)
...
class Decoder(nn.Module):
    """
    LSTM Decoder - Takes context vector from encoder and generates target sequence
    """
    ...
    def forward(self, input: torch.Tensor, hidden: torch.Tensor, cell: torch.Tensor):  # how to predict the next single word
        ...
        Args:
            input: Current input token (batch_size, 1) - single token
        ...
        """
        embedded = self.embedding(input)  # # Embed input token (batch_size, 1, embedding_dim)
        embedded = self.dropout(embedded)
...
class Seq2Seq(nn.Module):
    ...
    For long sentences, this single context vector cannot capture all information (Bottleneck problem)
    """
